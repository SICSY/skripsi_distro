...
import { Search, Eye, MoreHorizontal, Users, RefreshCw, ChevronLeft, ChevronRight, Download, Mail, Phone, MapPin, Package, Palette } from "lucide-react";
...
export default function CustomersPage() {
  ...
  const exportCustomers = () => {
    ...
    const csvContent = [
      ...
      ...data.data.customers.map((customer: any) => [customer.name, customer.email, customer.phone, customer._count.orders, customer.totalSpent, new Date(customer.createdAt).toLocaleDateString()].join(","))
    ].join("\n");
  ...
  return (
    <div className='space-y-6 p-6'>
      ...
      <Card className='bg-zinc-900 border-zinc-700'>
        <CardContent className='p-6'>
          <div className='relative'>
            ...
            <Input placeholder='Search customers by name, email, or phone...' value={search} onChange={(e) => setSearch(e.target.value)} className='pl-10 bg-zinc-800 border-zinc-600 text-white' />
          </div>
        </CardContent>
      </Card>
      ...
      <Card className='bg-zinc-900 border-zinc-700'>
        <CardHeader>
          <CardTitle className='text-white'>Customers {data?.data.pagination?.total ? `(${data.data.pagination.total})` : ""}</CardTitle>
                ...
                <TableBody>
                  {data?.data.customers?.map((customer: any) => (
                    <TableRow key={customer.id} className='border-zinc-700'>
                      ...
                      <TableCell className='font-medium text-white'>{formatCurrency(customer.totalSpent || 0)}</TableCell>
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align='end' className='bg-zinc-800 border-zinc-600'>
                            ...
                            <DropdownMenuItem onClick={() => openCustomerDetail(customer.id)} className='text-zinc-300 hover:bg-zinc-700'>
                              <Eye className='mr-2 h-4 w-4' />
                              View Details
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => window.open(`mailto:${customer.email}`)} className='text-zinc-300 hover:bg-zinc-700'>
      ...
      <Dialog open={isDetailDialogOpen} onOpenChange={setIsDetailDialogOpen}>
        <DialogContent className='bg-zinc-900 border-zinc-700 max-w-4xl'>
          <DialogHeader>
            ...
            <DialogDescription className='text-zinc-400'>Complete customer information and order history</DialogDescription>
          </DialogHeader>
          {customerDetail?.data && (
                ...
                <Card className='bg-zinc-800 border-zinc-700'>
                  ...
                  <CardContent className='space-y-4'>
                    <div className='flex justify-between'>
                      ...
                      <span className='text-white font-medium'>{customerDetail.data._count?.orders || 0}</span>
                    </div>
                    <div className='flex justify-between'>
                      <span className='text-zinc-400'>Total Spent</span>
                      <span className='text-white font-medium'>{formatCurrency(customerDetail.data.stats?.totalSpent || 0)}</span>
                    </div>
                    <div className='space-y-2'>
                      <span className='text-zinc-400 text-sm'>Orders by Status</span>
                      {customerDetail.data.stats?.ordersByStatus &&
                        ...
                        customerDetail.data.orders?.map((order: any) => (
                          <div key={order.id} className='flex items-center justify-between p-4 bg-zinc-700 rounded-lg'>
                            <div className='flex items-center gap-4'>
                              ...
                              {order.productKustom ? (
                                order.productKustom.photo ? (
                                  <img src={order.productKustom.photo || "/placeholder.svg"} alt={order.productKustom.name} className='w-12 h-12 rounded object-cover' />
                                ) : (
                                ...
                                order.product.images && order.product.images.length > 0 ? (
                                  <img src={order.product.images[0] || "/placeholder.svg"} alt={order.product.name} className='w-12 h-12 rounded object-cover' />
                                ) : (
                              ...
                              <div>
                                ...
                                <p className='text-sm text-zinc-300'>{order.productKustom?.name || order.product?.name || "Unknown Product"}</p>
                                <div className='flex items-center gap-2 text-xs text-zinc-400'>
                                  <span>{formatDate(order.createdAt)}</span>
                                  {order.quantity && <span>• Qty: {order.quantity}</span>}
                                  {order.design && <span>• {order.design.totalObjects} objects</span>}
                                </div>
                              </div>
                            </div>
                            <div className='text-right'>
                              <Badge className={`${statusColors[order.status as keyof typeof statusColors]} text-white mb-2`}>{order.status}</Badge>
                              <p className='text-white font-medium'>{formatCurrency(Number(order.totalAmount))}</p>
                            </div>
                          </div>
                        ))
                      )}
...
// import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuLabel, DropdownMenuSeparator, DropdownMenuTrigger } from "@/components/ui/dropdown-menu";
// import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from "@/components/ui/dialog";
// import { Search, Eye, MoreHorizontal, Users, RefreshCw, ChevronLeft, ChevronRight, Download, Mail, Phone, MapPin } from "lucide-react";
// import { useCustomers, useCustomer } from "@/src/hooks/use-admin";
// import { Skeleton } from "@/components/ui/skeleton";
// import { toast } from "sonner";
...
//                     <div className='flex justify-between'>
//                       <span className='text-zinc-400'>Total Spent</span>
//                       <span className='text-white font-medium'>{formatCurrency(customerDetail.data.stats.totalSpent)}</span>
//                     </div>
//                     <div className='space-y-2'>
//                       <span className='text-zinc-400 text-sm'>Orders by Status</span>
//                       {Object.entries(customerDetail.data.stats.ordersByStatus).map(([status, count]) => (
...
//                         <div key={order.id} className='flex items-center justify-between p-4 bg-zinc-700 rounded-lg'>
//                           <div className='flex items-center gap-4'>
//                             {order.product.photo && <img src={order.product.photo || "/placeholder.svg"} alt={order.product.name} className='w-12 h-12 rounded object-cover' />}
//                             <div>
//                               <p className='font-medium text-white'>#{order.orderId}</p>
